		...
		while True:
			...
			for i in range(4):
				if(i+1==rand):
					...
					texRectObj = texSurfaceObj.get_rect()
					pygame.draw.circle(DISPLAYSURF, GREEN, (foodx+(i+1)*gap, foody), 40, 0)
					texRectObj.center = (foodx+(i+1)*gap,foody+3)
					DISPLAYSURF.blit(texSurfaceObj, texRectObj)
				else:
					texSurfaceObj = fontObj.render (str(randomOptions[counter]) , True, BLACK )
					counter +=1
					texRectObj = texSurfaceObj.get_rect()
					pygame.draw.circle(DISPLAYSURF, GREEN, (foodx+(i+1)*gap, foody), 40, 0)
					texRectObj.center = (foodx+(i+1)*gap,foody+3)
					DISPLAYSURF.blit(texSurfaceObj, texRectObj)
			counter =0
			pygame.draw.rect(DISPLAYSURF, BLUE, (boxx-40,boxy,boxx-boxx+80,boxy))
